...
Data_f = {'data1': [1, 1],'data2': [0, 1], 'data3': [1, 0], 'data4':[1, 0], 'data5':[1, 0]}
features = DataFrame (Data_f, columns = ['data1','data2', 'data3', 'data4', 'data5'])

Data_l = {'output1': [0, 0], 'output2': [1, 0]}
labels = DataFrame (Data_l, columns = ['output1', 'output2'])

model = Sequential()
model.add(Dense(4, input_dim=5, activation='tanh'))
...
with open("memocode_aibro_c.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("memocode_aibro_c.h5")
