...
#ifndef tanh
	#define tanh(x)			DIV(\
								SUB(FROM_INT(1), EXP(MUL(FROM_INT(-2), x))),\
								ADD(FROM_INT(1), EXP(MUL(FROM_INT(-2), x)))\
							)
#endif
#ifndef sigmoid
...
#endif
#ifndef hard_sigmoid
	#define hard_sigmoid(x)		((x) > 2.5 ? 1 : ((x) < -2.5 ? 0 : ADD(0.5, MUL(x,0.2))))
#endif
...
#define ACT_ENUM_HARD_SIGMOID 5
